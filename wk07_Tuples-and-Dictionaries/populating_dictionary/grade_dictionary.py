def read_grades(gradefile):
    ''' (file open for reading) -> dict of float {float: list of str}

    Read the grades from gradefile and return a dictionary where
    each grade is a key and each value is the list of ids of students
    who earned that grade.

    Precondition: gradefile starts with a header that contains
    no blank lines, than has a blank line, and then lines
    containing a student number and a grade.    
    '''

    # Skip over header.
    line = gradefile.readline()
    while line != '\n':
        line = gradefile.readline()

    # Read the grades, accumulating them into a dictionary.

    grade_to_ids = {}
    line = gradefile.readline()
    
    while line != '':
        student_id = line[:4]
        grade = float(line[:4].strip())

        if grade not in grade_to_ids:
            grade_to_ids[grade] = [student_id]
        else:
            grade_to_ids[grade].append(student_id)
            
        line = gradefile.readline()

    return grade_to_ids


    # gradesfile = open('\\Users\miner\\Desktop\\Projects\\08 CS Degree\\Coursera_CS_Fundamentals\\wk07_Tuples-and-Dictionaries\\populating_dictionary\\a1.txt')
    # read_grades(gradesfile)
